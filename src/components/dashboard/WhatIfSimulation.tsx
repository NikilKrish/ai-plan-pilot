...
import { stationsByLine, type ScenarioRecord } from '@/data/sampleData';
...
const WhatIfSimulation = () => {
  ...
  const handleRealRun = () => {
    ...
    const adjustments = { shiftDelta: 1, workingHoursDelta: 0, downtimeDeltaPct: 0, scrapDeltaPct: 0 };
  ...
  return (
    <div className="bg-card rounded-3xl shadow-sm border border-border card-lift overflow-hidden col-span-1 md:col-span-2">
      <div className="h-[280px] bg-visual-area border-b border-border relative p-5 shadow-inner flex flex-col">
        <div className="flex items-center justify-between mb-3">
          <div className="flex bg-muted rounded-lg p-0.5">
            <button onClick={() => setActiveTab('scenario')} className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${activeTab === 'scenario' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}>Scenario</button>
            <button onClick={() => setActiveTab('compare')} className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${activeTab === 'compare' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}>Compare</button>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="text-[10px] font-medium px-2 py-1 rounded-md border border-border text-muted-foreground hover:bg-muted transition-colors">Share</button>
          </div>
        </div>

        {activeTab === 'scenario' && !showCompare ? (
          <div className="flex-1 overflow-hidden relative">
            <div className="flex gap-3 auto-scroll-track" style={{ width: `${allScenarios.length * 216}px`, maskImage: 'linear-gradient(90deg, transparent 0%, black 8%, black 92%, transparent 100%)', WebkitMaskImage: 'linear-gradient(90deg, transparent 0%, black 8%, black 92%, transparent 100%)' }}>
              {allScenarios.map((s, i) => (
                <div key={i} className="w-[200px] flex-shrink-0 bg-card rounded-2xl border border-border p-3 shadow-sm">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-2" style={{ backgroundColor: `hsl(var(--${s.color}-light))`, color: `hsl(var(--${s.color}))` }}>{icons[s.icon]}</div>
            ...
            {['Shift +1', 'Reduce Hrs', 'Rebalance'].map((label, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full flex gap-1 items-end justify-center h-24">
                  <div className="w-5 rounded-t transition-all duration-700" style={{ height: `${[70, 45, 60][i]}%`, backgroundColor: 'hsl(var(--accent-blue))' }} />
                  <div className="w-5 rounded-t transition-all duration-700" style={{ height: `${[85, 55, 75][i]}%`, backgroundColor: 'hsl(var(--accent-green))' }} />
                </div>
        ...
        {showToast && (
          <div className="absolute bottom-16 right-5 bg-foreground text-primary-foreground text-[11px] font-medium px-3 py-2 rounded-lg shadow-lg" style={{ animation: 'toastIn 0.3s ease-out' }}>
      ...
      <div className="p-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">Simulate multiple scenarios in seconds (shifts, hours, model mix) and compare outcomes.</p>
      </div>
    </div>
  );
};
